...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [] }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      mapRef.current = map;
      // Create icons
      createIcon(map, 'plane-cyan', '#39FF14', 24);
      ...
      createDot(map, 'dot-gold', '#448AFF', 8);
      ...
      }, paint: { 'text-color': '#448AFF', 'text-halo-color': '#000', 'text-halo-width': 1, 'text-opacity': 0.85 }});
      ...
      map.on('click', layer, e => {
        ...
        popup(coords, `<div style="${pStyle}border:1px solid rgba(68,138,255,0.3);">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
            <span style="color:#448AFF;font-size:16px;font-weight:700;letter-spacing:0.1em;">${cs}</span>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:11px;">
            ...
            <div><span style="color:#5C5A54;font-size:9px;">ALT</span><br/><span style="color:#39FF14;">${p.alt?Math.round(p.alt)+'m':'—'}</span></div>
          ...
          <div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap;">
            <a href="https://www.flightaware.com/live/flight/${cs}" target="_blank" style="${linkStyle}color:#448AFF;border:1px solid rgba(68,138,255,0.4);background:rgba(68,138,255,0.1);">⚡ FLIGHTAWARE</a>
            <a href="https://globe.adsbexchange.com/?icao=${p.icao24||''}" target="_blank" style="${linkStyle}color:#39FF14;border:1px solid rgba(57,255,20,0.4);background:rgba(57,255,20,0.1);">📡 ADS-B</a>
    ...
    map.on('click', 'sat-dots', e => {
      ...
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(68,138,255,0.3);">
        <div style="color:#448AFF;font-size:12px;font-weight:700;letter-spacing:0.1em;margin-bottom:4px;">🛰️ ${p.name}</div>
        ...
        <a href="https://www.n2yo.com/?s=${encodeURIComponent(p.name||'')}" target="_blank" style="${linkStyle}color:#448AFF;border:1px solid rgba(68,138,255,0.4);background:rgba(68,138,255,0.1);">🔭 TRACK ON N2YO</a>
      </div>`);
    });
    ...
    map.on('click', 'scan-targets-dots', (e: any) => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,61,61,0.5);">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9px;">
          <div><span style="color:#5C5A54;">TYPE</span><br/><span style="color:#39FF14;">${(p.type || 'UNKNOWN').toUpperCase()}</span></div>
          <div><span style="color:#5C5A54;">COORDS</span><br/><span style="color:#E8E6E0;">${coords[1].toFixed(3)}°, ${coords[0].toFixed(3)}°</span></div>
        </div>
      </div>`);
    });
    ...
    map.on('click', 'weather-dots', e => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(224,64,251,0.3);">
        ...
        <div style="display:flex;gap:6px;">
          ...
          <a href="https://eonet.gsfc.nasa.gov/api/v3/events/${p.id || ''}" target="_blank" style="${linkStyle}color:#448AFF;border:1px solid rgba(68,138,255,0.4);background:rgba(68,138,255,0.1);">🛰️ NASA EONET</a>
        </div>
      </div>`);
    });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (projection === 'globe') {
        ...
        } catch (e) { console.warn('[CRYPTIS] Suppressed error:', e instanceof Error ? e.message : e); }
      } else {
