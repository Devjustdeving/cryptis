...
export default function SharePanel({ mapView, activeLayers, mouseCoords }: SharePanelProps) {
  ...
  const generateShareUrl = useCallback(() => {
    ...
    const base = typeof window !== 'undefined' ? window.location.origin : 'https://cryptis.vercel.app';
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-12 right-0 w-72 glass-panel p-4 pointer-events-auto cryptis-glow z-[300]"
          >
            ...
            <div className="flex gap-2">
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent('🏛️ CRYPTIS — Global Intelligence Dashboard')}&url=${encodeURIComponent(generateShareUrl())}`}
                target="_blank"
              ...
              <a
                href={`https://reddit.com/submit?url=${encodeURIComponent(generateShareUrl())}&title=${encodeURIComponent('CRYPTIS — Open Source Global Intelligence Platform')}`}
                target="_blank"
