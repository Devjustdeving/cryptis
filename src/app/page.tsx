...
const CryptisMap = dynamic(() => import('@/components/OsirisMap'), { ssr: false });
...
export default function Dashboard() {
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    } catch (e) { console.warn('[CRYPTIS] Suppressed error:', e instanceof Error ? e.message : e); } finally { setDossierLoading(false); }
  }, []);
  ...
  const fetchEndpoint = useCallback(async (url: string, transform?: (d: any) => any, options?: RequestInit) => {
    ...
    try {
      ...
      console.warn('[CRYPTIS] Suppressed error:', e instanceof Error ? e.message : e);
    ...
    <main className={`fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden ${focusMode ? 'focus-mode' : ''}`}>
      ...
      {!isMobile && (
        ...
        >
          ...
          <div className="flex items-center gap-3 min-w-[200px]">
            ...
            <a href="https://x.com/CryptisOSIP" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-6 h-6 rounded-sm border border-[var(--border-primary)] bg-[var(--bg-secondary)] hover:border-[var(--gold-primary)]/40 hover:bg-[var(--gold-primary)]/10 transition-colors" title="Follow on X">
              <svg viewBox="0 0 24 24" className="w-3 h-3 fill-[var(--text-muted)] hover:fill-[var(--text-primary)] transition-colors"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
          </div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#111] border-b border-[var(--border-primary)]">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#FF4081] animate-cryptis-pulse" />
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[9px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'CRYPTIS RECON' : 'SEARCH'}
                    </span>
      ...
      {(regionDossier || dossierLoading) && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute top-16 md:top-20 left-2 right-2 md:left-1/2 md:right-auto md:-translate-x-1/2 z-[300] md:w-[480px] max-h-[65vh] overflow-y-auto styled-scrollbar">
          <div className="glass-panel p-5 cryptis-glow">
