# Self-Hosting CRYPTIS with Docker

CRYPTIS ships as a self-contained Next.js standalone build. This guide covers
running it with Docker / Docker Compose, deploying it as a [CasaOS](https://casaos.io)
app, and configuring the optional API keys.

> **TL;DR:** CRYPTIS runs fully **without any API keys**. All core feeds
...
- **`env_file: .env` (`required: false`)** — if a `.env` file exists its
  values are injected into the container; if it's missing, CRYPTIS still starts
  with the keyless feeds.
- **`ports: ${CRYPTIS_PORT:-3000}:3000`** — the web UI. The container always
  listens on 3000; the published **host** port is `CRYPTIS_PORT` (default
  `3000`). Set `CRYPTIS_PORT` in `.env` to remap it, e.g. `CRYPTIS_PORT=3005`
...
Registry, so you can run CRYPTIS without building anything:

```bash
docker pull ghcr.io/aiacos/cryptis:latest      # or a pinned tag, e.g. :0.1.0
docker run -d --name cryptis \
  -p 3005:3000 --env-file .env --restart unless-stopped \
  ghcr.io/aiacos/cryptis:latest
```
...
```bash
docker build -t cryptis:latest .
docker run -d --name cryptis -p 3000:3000 --env-file .env --restart unless-stopped cryptis:latest
```
...
1. On the CasaOS host, clone the repo somewhere persistent (e.g.
   `/DATA/AppData/cryptis`).
2. CasaOS dashboard → **`+`** → **Install a customized app** → paste the
   contents of `docker-compose.yml`.
   *(or simply run `docker compose up -d` from the cloned directory).*
3. CRYPTIS appears on the dashboard with its icon, reachable on host port
   `3000` (or whatever `CRYPTIS_PORT` you set in `.env`).
...
> directly, either build/tag `cryptis:latest` first
> (`docker build -t cryptis:latest /path/to/cryptis`) or set `image:` to a
...
| `SCANNER_KEY` | Shared secret; **must equal the backend's `CRYPTIS_KEY`** | RECON toolkit |

Without `SCANNER_URL`/`SCANNER_KEY` the RECON endpoints return `503` and the
rest of CRYPTIS works normally. Generate a key with `openssl rand -hex 32`.
